...
my $cgi_script = "\/cgi-bin\/ffl_2015\/players.cgi";
print header();
...
if(param( 'f_team' ))
{
  ...
  {
    $sql = "select player,ffl_team,total_pts,value from ffl_2015.$playerDB order by total_pts desc;";
  }
  elsif($team eq 'all_teams')
  {
    $sql = "select player,ffl_team,total_pts,value from ffl_2015.$playerDB where ffl_team <> '' order by total_pts desc;";
  } 
  else
  {
    $sql = "select player,ffl_team,total_pts,value from ffl_2015.$playerDB where ffl_team = '$team' order by total_pts desc;";
  }
