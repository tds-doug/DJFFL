...
my $cgi_script = "\/cgi-bin\/ffl_2015\/req_transaction.cgi";
...
if(param( 'f_player'))
{
  ...
  my $sql_action = "insert into transaction_request values ($week, $count +1, '$ffl_team', '$action', '$name','$nfl_team','$position', '$transactiontime');";
...
# Begin Your Movies for Week # Section
  ...
  my $sql_trans = "select action, player, nfl_team, position from transaction_request where week = $week and ffl_team = '$ffl_team' order by number desc;";
...
# Number teams requesting bit
    ...
    my $sql_num = "select count(*) from transaction_request where week = $week and player = '$row->[1]';";
...
elsif(param( 'f_action' ))
{
  ...
  print "<h3>Select a Player to $action</h3>\n";
...
# Begin Your Movies for Week # Section
  print "<h3><center>Your Moves for Week $week</center></h3>\n";
