...
print header();
print start_html("Fantasy Football Minimum Wage Earners");

print "<body bgcolor=\"#132C06\" link=\"#FFFF80\" vlink=\"#FFFF80\" alink=\"#FFFF80\">\n";
print "<font face=\"Verdana\" color=\"#FFFFDF\">\n";

print "<h1><center><font face=\"Verdana\" color=\"#FFFF80\">No One Can Live On California Minimum Wage</font></center></h1>\n";

print "<h3><center><font face=\"Verdana\" color=\"#FFFF80\">\$1 Players</font></center></h3>\n";
...
my $sql = "(select player,nfl_team,position,ffl_team,total_pts from players_QB where value = 1 and total_pts > 0)"
        . " union (select player,nfl_team,position,ffl_team,total_pts from players_WR where value = 1 and total_pts > 0)"
        . " union (select player,nfl_team,position,ffl_team,total_pts from players_RB where value = 1 and total_pts > 0)"
        . " union (select player,nfl_team,position,ffl_team,total_pts from players_TE where value = 1 and total_pts > 0)"
        . " union (select player,nfl_team,position,ffl_team,total_pts from players_K where value = 1 and total_pts > 0)"
