...
my $cgi_script = "\/cgi-bin\/ffl_2015\/admin_activate.cgi";
print header();
print start_html("FFL Admin - Activate Player from IR");
...
print "<h1><center><font face=\"Verdana\" color=\"#FFFF80\">Player Activate Control Page</font></center></h1>\n";
...
if(param( 'f_player' ))
{
  ...
  my $sql_delete_ir = "delete from injured_reserve where player  = '$name' and ffl_team = '$ffl_team';";
  my $sth_delete_ir = $dbh->prepare($sql_delete_ir);
  $sth_delete_ir->execute();
  ...
  my $sql_upd_trans = "insert into transactions values ($week, $number + 1, '$ffl_team','Activate','$name','$nfl_team','$position');";
...
# for the rest of the regular season change the $i <= $week to $i <= 13
  ...
  print "ir sql is $sql_delete_ir<p>\n";
...
elsif(param( 'f_ffl_team' ))  
{
  ...
  print "The following players are on the IR for $ffl_team<p>\n";
  ...
  my $sql_ir = "select player, position, nfl_team from ffl_2015.injured_reserve where ffl_team = '$ffl_team';";
...
{
  print "Please select the team you are about to activate a player for week <b><font color=\"#FF0000\">$week</font></b>.<p>\n";
